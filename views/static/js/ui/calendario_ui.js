...
export function mostrar_dias_calendario(div_dias,primeiro_dia,ultimo_dia,mes,ano){
...
export function atualiza_cabecalho_calendario(mes_cima,ano_cima,ano_calendario_cima,meses,mes,ano){
...
export function atualizar_dia_selecionado(diaCima,mostraDiaSemana,diaSemana,diasSemana,dia,mes) {
    diaCima.textContent = dia;

    const indice = new Date(
        Number(diaSemana.dataset.ano),
        mes - 1,
        Number(dia)
    ).getDay();
